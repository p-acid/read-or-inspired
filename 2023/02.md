...
```ts
type Reverse<T extends unknown[], R = T> = R extends [
  infer First,
  ...infer Rest
]
  ? [...Reverse<Rest>, First]
...
- Flip Arguments
  - 함수 `arguments` 의 타입은 배열입니다

```typescript
type ReversedArray<P = unknown[]> = P extends [infer First, ...infer Rest]
...
type FlipArguments<T extends Function> = T extends (
  ...
  ? (...args: ReversedArray<Args>) => Return
  : never;
```
...
- InorderTraversal (Failed)
  - 문제 유형 이해가 어려워 답안 참조
  - inorder traversal: 중위순회
    - 왼쪽 끝까지 내려간 이후 노드를 방문하고 오른쪽 자식노드로 이동하여 순회를 계속하게 된다.
  - 답안 해석
    - `left` 필드가 `TreeNode` 타입이면, 뒤에 `val` 값을 할당하고 반대의 경우(`right` 가 `TreeNode`) 앞에 `val`
...
> = T extends TreeNode
  ? T["left"] extends TreeNode
    ? [...InorderTraversal<T["left"], C>, T["val"]]
    : T["right"] extends TreeNode
    ? [T["val"], ...InorderTraversal<T["right"], C>]
    : [...C, T["val"]]
  : C;
```
...
type Flip<T extends object> = {
  [Key in keyof T as T[Key] extends KeyType
    ? T[Key]
...
- Fibonacci (failed)
  - `T` 외의 3가지 인수 추가
    - `Arr1`, `Arr2`: 피보나치 수열을 구현하기 위한 장치 인자
      - 비교가 실패하면, 이전 배열 길이를 누산한 `unknown` 배열을 생성
    - `StepArr`: `T` 번째 피보나치 수를 구하기 위해 비교하는 카운터
...
type Fibonacci<
  ...
  StepArr extends unknown[] = [unknown]
> = StepArr["length"] extends T
  ? Arr2["length"]
  : Fibonacci<T, [...Arr2], [...Arr1, ...Arr2], [...StepArr, unknown]>;
```
...
- AllCombinations (failed)
  - 유니온 형태로 만드는 타입 유틸(`StringToUnion`) + 유니온 타입의 조합을 만드는 타입 유틸(`Combinations`)
  - `<T extends string, U = T>`: 여기서 `U = T` 타입은 유니온 타입 내 특정 타입을 지정하기 위함이다.
...
- Greater Than (failed)
  - 이하 해석 작성
  - [참고 해답 링크](https://github.com/type-challenges/type-challenges/issues/23142)
...
/**
 * 단일 자릿수 비교
 * `Index` 배열 타입 내에 요소를 하나씩 추가하며 자릿수를 비교
 * `T` 가 우선적으로 매치되면(T가 더 작으면), `false` 반환. 반대의 경우 `true`
 * 둘 다 해당하지 않으면 `Index` 배열 추가
 */
type CompareDigit<
  T extends string,
  U extends string,
...
/**
 * 숫자 내 각 자릿수 비교
 * 제일 높은 자릿수부터 차례대로 비교를 진행하면서, 비교가 참으로 나오면(전자가 후자보다 크다면) 바로 `true` 를 반환
 * 그렇지 않다면 재차 비교를 진행하고 최종적으로 `false` 반환
 */
...
/**
 * 숫자 크기 비교 함수
 * 숫자 길이가 같은 경우: `CompareDigitArray` 를 통해 비교 진행
 * 그렇지 않은 경우: 숫자 길이 비교 결과 반환 ➡️ 숫자 길이가 두 자릿수로 나와도 재귀를 통해 재비교
 */
type GreaterThan<
  T extends number,
  U extends number
...
- IsTuple (failed)
  - `number extends T['length']` 분기 부분이 이해가 어렵다.
  - 추측하건데, 튜플 형태가 아닌 배열(예를 들어, `number[]`)의 경우 `length` 타입이 `number` 이기에, 특정 숫자로 결정되는 튜플의 `length` 는 `number extends T['length']` 에 해당하지 못해서 그런거 같다.
...
- Chunk (failed)
  - `U`: 최종 결과로 스태킹 되는 배열 타입
  - `C`: 제한만큼 요소를 추가하는 2차원 배열에 해당하는 타입
...
- Fill (failed)
  - `Limit` 값이 `Start` 와 `End` 값에 `extends` 될 때마다 바꿔주는 방식으로 진행
    - `Start` 도달 전: 순환 배열(`T`) 앞 요소를 결과 배열(`U`)에 하나씩 추가
    - `Start` 도달: `End` 값으로 `Limit` 변경
    - `End` 도달 전: 결과 배열에 `N` 을 추가
    - `End` 도달: `Limit` 값을 다시 `Start` 로 변경
    - `End` 이후: 남은 순환 배열 앞 요소를 결과 배열(`U`)에 하나씩 추가
...
> = Start extends End
  ? T
  : U["length"] extends Limit
  ? Fill<T, N, Start, End, U, Limit extends Start ? End : Start>
  : T extends [infer F, ...infer R]
  ? Fill<R, N, Start, End, [...U, Limit extends End ? N : F], Limit>
  : U;
```
...
type Trunc<N extends number | string> =
  `${N}` extends `${infer Integer}.${string}` ? Integer : `${N}`;
```

- IndexOf (hint)

```ts
type IndexOf<T, U, I extends unknown[] = []> = T extends [
  infer First,
  ...infer Rest
]
  ? Equal<U, First> extends true
    ? I["length"]
    : IndexOf<Rest, U, [...I, First]>
  : -1;
```

- Join (failed)
  - [참고 링크](https://github.com/type-challenges/type-challenges/issues/23310)
...
type Join<
  T,
  U extends ArrayElement,
  Result extends ArrayElement = ""
> = T extends [
  infer First extends ArrayElement,
  ...infer Rest extends ArrayElement[]
]
  ? Join<Rest, U, `${Result}${Result extends "" ? "" : U}${First}`>
  : Result;
```
...
### [How to check type in typescript : Type Guard](https://radlohead.gitbook.io/typescript-deep-dive/type-system/typeguard#instanceof)

> 타입스크립트에서 타입 가드(Type Guard)를 활용하면 타입을 좁힐 수 있다.

- `typeof` 를 통한 비교 : `typeof` 연산자를 통해 해당 값의 타입 스트링을 반환하여 이를 통해 비교
...
- `instanceof` 를 통한 비교 : `instanceof` 연산자를 통해 특정 타입과 이를 통해 비교

  ```ts
  type MixedType = File | { name: string; size: number };

  const data: MixedType = new File();

  if (data instanceof File) {
    console.log("This is file data.");
  } else {
    console.log("This is file liked data.");
  }
  ```

- `in` 를 통한 비교 : `in` 연산자를 통해 객체 내부에 특정 필드가 존재하는지 여부를 파악
...
- Unique (failed)
  - 비교할 인자(`U`)가 배열(`T`) 내에 존재하는지 비교하는 `Exist` 타입 함수
  - 최종적으로 `Unique` 타입 함수는,
    - 인자 배열(`T`)을 순환하면서 두 번째 인자인 결과 배열(`Acc`)에 요소가 존재하는지 `Exist` 함수로 비교를 진행하고,
    - 없다면(`-1` 일 경우), 결과 배열에 추가한 상태로 재귀
    - 있다면(그 외), `Acc` 배열을 그대로 할당하여 재귀
    - 순환 종료 시, `Acc` 반환

```ts
type Exist<T, U> = T extends [infer F, ...infer R]
...
type MapTypes<T, R extends ReturnStructure> = {
  [Key in keyof T]: T[Key] extends R[`mapFrom`]
    ? R extends { mapFrom: T[Key]; mapTo: infer To }
      ? To
      : never
    : T[Key];
};
...
- Number Range (failed)
  - 3번째 유형만 `Type instantiation is excessively deep and possibly infinite` 에러로 실패

```ts
// successful

type ArrayToUnion<A extends unknown[]> = A extends [infer F, ...infer R]
  ? F | ArrayToUnion<R>
  : never;

type ConstructTuple<L, R extends number[] = []> = R["length"] extends L
  ? [...R, L]
  : ConstructTuple<L, [...R, R["length"]]>;

type NumberRange<
  L extends number,
  H extends number,
  R extends number[] = ConstructTuple<H>
> = R extends [infer First, ...infer Rest extends number[]]
  ? First extends L
    ? ArrayToUnion<R>
    : NumberRange<L, H, Rest>
  : ArrayToUnion<R>;
```
...
type NumberRange<
  ...
  H extends number,
  R extends number = L,
  CountArray extends unknown[] = ConstructTuple<L>
> = CountArray["length"] extends H
  ? R | CountArray["length"]
  : NumberRange<L, H, R | CountArray["length"], [...CountArray, unknown]>;
```

- Combination (failed)

```ts
type ToUnion<T extends string[]> = T extends [
  infer F extends string,
  ...infer R extends string[]
]
  ? F | ToUnion<R>
  : never;
type UnionComb<T extends string, U = T> = U extends T
  ? U | `${U} ${UnionComb<Exclude<T, U>>}`
  : ``;
...
type DuplicatedUnion<
  S extends string,
  U extends string = never
> = S extends `${infer F}${infer R}`
  ? F extends U
    ? F | DuplicatedUnion<R, U>
    : DuplicatedUnion<R, U | F>
  : never;

type FirstUniqueCharIndex<T extends string, C extends unknown[] = []> = Equal<
  StringToUnion<T>,
  DuplicatedUnion<T>
> extends true
  ? -1
  : T extends `${infer F}${infer R}`
  ? F extends DuplicatedUnion<T>
    ? FirstUniqueCharIndex<R, [...C, unknown]>
    : C["length"]
  : -1;
```

- GetMiddleElement (failed)
  - 좌, 우측 요소들을 제거하면서 도출해내는 방식
...
type GetPrimitive<T, A extends unknown[] = Primitive> = A extends [
  infer F,
  ...infer R
]
  ? T extends F
...
type ToPrimitive<T> = {
  [Key in keyof T]: GetPrimitive<T[Key]> extends false
    ? ToPrimitive<T[Key]>
    : GetPrimitive<T[Key]>;
};
```
...
```ts
type Value = string | number | boolean | unknown[] | Function;

type DeepMutable<T extends { [K: PropertyKey]: any }> = {
  -readonly [Key in keyof T]: T[Key] extends Value
    ? T[Key]
    : DeepMutable<T[Key]>;
};
```
...
- All
  - 나의 답안: `Equal` 타입 유틸을 순환하며 비교
  - 다른 답안: `T` 배열의 전체 타입과 타입 `P` 의 합집합이 `P` 와 일치하는지 확인
    - 다른 타입이 포함될 경우 일치할 수 없음

```ts
// My answer
type All<T extends unknown[], C> = T extends [infer F, ...infer R]
