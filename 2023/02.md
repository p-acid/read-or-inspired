...
```typescript
type ReversedArray<P = unknown[]> = P extends [infer First, ...infer Rest] ? [...ReversedArray<Rest>, First] : P

type FlipArguments<T extends Function> = T extends (...args: infer Args) => infer Return  ? (...args: ReversedArray<Args>) => Return : never
```
...
type BEM<B extends string, E extends string[], M extends string[]> = Length<E> extends 0 
  ? `${B}--${M[number]}` 
  : Length<M> extends 0
  ?  `${B}__${E[number]}`
 ...
 */
type CompareDigit<T extends string, U extends string, Index extends any[] = []> = T extends DigitArray[Index['length']] ? false : U extends DigitArray[Index['length']] ? true : CompareDigit<T, U, [...Index, any]>
/** 
 * 숫자 내 각 자릿수 비교
 ...
 */
type GreaterThan<T extends number, U extends number> = NumberLength<T> extends NumberLength<U> ? CompareDigitArray<StringToArray<`${T}`>, StringToArray<`${U}`>> :  GreaterThan<NumberLength<T>, NumberLength<U>>
```
...
```ts
type ToUnion<T extends unknown | unknown[]> = T extends [infer First, ...infer Rest] ? First | ToUnion<Rest> : T
...
```ts
type Trunc<N extends number | string> = `${N}` extends `${infer Integer}.${string}` ? Integer : `${N}`
```

- IndexOf (hint)

```ts
type IndexOf<T, U, I extends unknown[] = []> = T extends [infer First, ...infer Rest]
...
type Join<T, U extends ArrayElement, Result extends ArrayElement = ''> = T extends [infer First extends ArrayElement, ...infer Rest extends ArrayElement[]] 
